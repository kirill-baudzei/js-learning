...
function returnNeighboringNumbers(number) {
  const result = [number - 1, number, number + 1];
  return result;
}

// решение 3
function getMathResult(numberBas, repetionNum) {
  if (typeof repetionNum === "number" && repetionNum > 0) {
    let result = "";
    for (let i = 1; i <= repetionNum; i++) {
      result += numberBas * i;
      if (i !== repetionNum) {
        result += "---";
      }
